...
### 16. `nusc_vis_pred.py`: カメラ画像への地図要素の投影描画機能

予測(PRED)・GTの地図要素は、従来は俯瞰図(`PRED_MAP_plot.png`/`GT_fixednum_pts_MAP.png`)
にしか描画されておらず、カメラ画像自体には重畳描画されていなかった。カメラ画像への
投影描画機能を追加した。実装・デバッグの過程で以下の3点の補正が必要だった:

- **z座標(地面の高さ)**: 地図要素の座標はLiDARセンサー原点(z=0)基準だが、実際の路面は
  LiDARの設置高さ分(車両により異なる、実測で約1.6〜1.84m)下にある。`z=0`のまま投影すると
  線が宙に浮いて見える。`img_metas['lidar2ego']`の並進成分から**サンプルごとに実際の高さを
  動的取得**するようにした(`get_ground_z()`。取得できない場合のみ`-1.6`にフォールバック)。
- **投影先の画像**: `lidar2img`行列はネットワーク入力用にリサイズ・パディングされた画像に
  対応しているため、生のJPEG(`CAM_FRONT.jpg`等、解像度が異なる)ではなく、そのリサイズ・
  パディング画像を`img_norm_cfg`の`mean`/`std`/`to_rgb`で復元(`denorm_cam_img()`)した
  ものに描画するようにした。そのため出力画像は生画像と解像度・アスペクト比が異なり、
  下端に(size_divisorでパディングされた分の)無地の帯が写るのは仕様であり不具合ではない。
- **カメラの並び順**: `img`テンソルのカメラ順序は、データセット変換スクリプトの
  `camera_types`順(`CAM_FRONT, CAM_FRONT_RIGHT, CAM_FRONT_LEFT, ...`)であり、
  表示レイアウト用の`CAMS`定数の順序とは異なる。`enumerate(CAMS)`で`img[0, cam_idx]`を
  取り出すとカメラ内容とファイル名がずれるため、実際の対応関係を
  `img_metas[0]['filename']`から都度導出するようにした。

投影ロジックが正しいことの検証のため、予測結果だけでなくGTも同じ経路で投影する
デバッグ機能も恒久的に追加した(GTが実際の車線・縁石に正しく重なることを確認済み)。

- [tools/maptrv2/nusc_vis_pred.py](../MapTR/tools/maptrv2/nusc_vis_pred.py):
  - `perspective()`, `colors_plt_to_bgr()`, `denorm_cam_img()`, `draw_lines_on_cam_img()`,
    `get_ground_z()` を追加
  - メインループで予測結果を`CAM_*_PRED.jpg`/`surroud_view_PRED.jpg`として、
    GTを`CAM_*_GTPROJ.jpg`/`surroud_view_GTPROJ.jpg`として各サンプルディレクトリに出力
  - 既存の出力(`CAM_*.jpg`、`surroud_view.jpg`、`PRED_MAP_plot.png`等)はそのまま維持して
    おり、`generate_video.py`など既存の後続処理に影響なし
  - カメラ画像・BEV俯瞰図の両方に色⇔クラス名のLegend(凡例)を追加
    (`draw_legend_cv2()`, `add_bev_legend()`。クラス名は`dataset.MAPCLASSES`から取得)

### 17. `nusc_vis_pred.py`: 坂道・車体傾斜がある場面での投影ズレ(試みたが撤回)

項番16の投影機能について、平坦な道(Singapore)のサンプルでは正しく重なったが、
坂の多いBoston市街地のサンプルでは、特に遠方の要素が実際の路面よりずれて投影される
現象が確認された。

原因の仮説: 地図の(x, y)座標は、車両姿勢の**ヨー角のみ**を使って構築された「水平フラット
平面」を前提にしている(`gen_vectorized_samples`が`quaternion_yaw()`のみを使用)一方、
`lidar2img`による投影は、ピッチ・ロールを含む**実際のLiDARの3D姿勢**を使う。坂道や
加減速による車体の傾きがある場面ではこの2つの基準面がずれるため、投影位置が
(特に遠方ほど大きく)ずれるのではないかと推測した。

**この仮説に基づき`img_metas['lidar2global']`の回転行列を使った補正
(`get_ground_plane_rotation()`)を追加したが、実際に検証したところ前方カメラでは
手前に寄りすぎ、後方カメラでは逆に遠方ほど浮き上がるという、前後で逆方向の誤差が
生じ、無補正の状態より悪化した。** ピッチ/ロールの符号の取り扱いに誤りがあった可能性が
高く、これ以上推測で符号を追うと悪化を繰り返すリスクが高いため、**この回転補正機能は
撤回**し、項番16の`ground_z`のみの補正(水平な地面を仮定)に戻した。

- 現状の既知の制限: 坂道や車体が大きく傾いている場面では、`nusc_vis_pred.py`の
  カメラ投影(特に遠方の要素)は実際の路面からややズレる場合がある。地図データ自体が
  車両のピッチ・ロール情報を保持しない(ヨー角のみのフラット表現)ための構造的な限界であり、
  平坦な道路では問題にならない。

### 18. カメラ投影の根本原因分析: なぜ完全なオーバーレイが難しいか

項番17の失敗を受けて、そもそもの限界を整理した考察。

**問題の本質**: 地図データ(GT/予測とも)は本質的に2次元(x, y)のみで、標高(z)情報を
一切持たない。

- `gen_vectorized_samples`が地図パッチを切り出す際に使うのは車両姿勢の**ヨー角のみ**
  (`quaternion_yaw(rotation)`)。ピッチ・ロールは最初から捨てられている。
- 地図データそのもの(nuScenesのHDマップ)が完全にフラットな2Dベクタレイヤであり、
  標高モデル(DEM)を持たない。道路が坂であっても勾配情報はどこにも記録されていない。

2D点群を3Dカメラ画像に投影するには、必ず「地面の高さ(z)」を何らかの形で仮定する必要が
あり、これまで試した2案は:

1. `ground_z`(定数)のみ → 平坦な道では良好(Singaporeサンプルで確認済み)
2. 車体姿勢(ピッチ)を使った回転補正(項番17) → 悪化して撤回

**②が失敗した理由**: `ego2global`/`lidar2global`の回転が捉えているのは、その瞬間の
**車両本体の姿勢**(サスペンションの沈み込み、加減速による前後ピッチ、カーブでのロール
など)であり、**その先の道路がどれだけ傾斜しているか**とは別物。急ブレーキ中の車は
平坦な道でも前のめりにピッチするが、それは道路の勾配ではない。逆に、遠方の要素の実際の
路面高さは、車両直下の姿勢からは分からない(路面は場所によって高さが変わるが、車両姿勢は
1つのスカラー値しかない)。「今の車の姿勢」を全地図点に一律適用するのは物理的に正しい
モデルではなく、近距離では誤差が小さいが遠距離・前後カメラで誤差の出方が矛盾する
(項番17の症状)。

**完全なオーバーレイに必要な情報**: 理想的には、各地図点(x, y)における実際の路面の
高さ(z)が必要。nuScenesのHDマップ自体はこれを持たないが、代替として:

- その場面のLiDAR点群(`LIDAR_TOP`の生データ)を使い、各地図点の(x, y)近傍にある
  実測LiDAR点のzの平均(または局所的な地面平面フィッティング)を高さとして使う案が
  考えられる。「今の車の姿勢」ではなく「その地点の実際の地面」を使うため、坂道でも
  理論上は正しく機能するはずだが、点群の読み込み・最近傍探索の追加実装が必要で未実装。
- または、Depth推定等の他センサ情報を使う手も考えられるが同様に未実装。

**結論**:
- 平坦な道では`ground_z`のみのアプローチ(項番16)で十分実用的。
- 坂道や複雑な地形では、車両姿勢だけからの補正には原理的な限界があり、単純な数式
  修正では解決できない。
- 完全なピクセル単位のオーバーレイは、標高情報を持たない2D地図データの性質上、
  根本的にはLiDAR点群等の追加データソースがない限り困難(既知の制限として受容する)。
